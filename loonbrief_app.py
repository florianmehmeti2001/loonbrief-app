...
# 1. Standaard waarden dictionary (zonder vast contract)
standaard_waarden = {
    'statuut': 'Extra (Horeca)',
    'uurloon': 15.97,
    'hotel': 'JA',
    'kledij_aantal': 2,
    'declaraties': 0.0,
    ...
    'feestdag': 'NEE',
    # Heenrit
    'h_shift': 'PRG', 'h_f1': 'TM', 'h_f2': 'Conducteur',
    'h_start': time(19, 0), 'h_einde': time(11, 0),
    'h_prsbln': 0, 'h_bruprg': 1,
    # Terugrit
    't_shift': 'PRG', 't_f1': 'TM', 't_f2': 'Conducteur',
    't_start': time(16, 30), 't_einde': time(11, 0),
    't_prsbln': 0, 't_bruprg': 1,
}
...
with st.sidebar:
    ...
    st.selectbox("Kies je statuut", ["Extra (Horeca)", "Flexi", "Student"], key='statuut')
    st.number_input("Basis Uurloon (€)", value=15.97, step=0.10, key='uurloon')
    st.radio("Hotelovernachting?", ["JA", "NEE"], key='hotel')
    st.radio("Feestdag?", ["JA", "NEE"], key='feestdag')
    st.checkbox("Valt deze shift op een Zondag?", key='is_zondag')
...
# 2. Urenberekening functie
def bereken_rit_uren(shift, start, einde, is_weekend_of_feestdag):
...
is_speciale_dag = st.session_state.is_zondag or (st.session_state.feestdag == "JA")

h_w, h_p, h_150, h_200 = bereken_rit_uren(st.session_state.h_shift, st.session_state.h_start, st.session_state.h_einde, is_speciale_dag)
t_w, t_p, t_150, t_200 = bereken_rit_uren(st.session_state.t_shift, st.session_state.t_start, st.session_state.t_einde, is_speciale_dag)
...
atm_count = (1 if st.session_state.h_f1 == "ATM" else 0) + (1 if st.session_state.t_f1 == "ATM" else 0)
tm_count = (1 if st.session_state.h_f1 == "TM" else 0) + (1 if st.session_state.t_f1 == "TM" else 0)
bru_count = st.session_state.h_bruprg + st.session_state.t_bruprg
prs_count = st.session_state.h_prsbln + st.session_state.t_prsbln

zondag_premie_aantal = 1 if st.session_state.is_zondag else 0
feestdag_premie_aantal = 1 if st.session_state.feestdag == "JA" else 0
...
totaal_loon = netto_loon + totaal_vakantiegeld

# Grote totaal weergave bovenaan
st.markdown("---")
col_m1, col_m2, col_m3 = st.columns(3)
col_m1.metric("💵 Totaal Loon (Netto + Vakantiegeld)", f"€ {totaal_loon:.2f}")
col_m2.metric("💰 Netto Loon", f"€ {netto_loon:.2f}")
col_m3.metric("📈 Bruto Totaal", f"€ {bruto:.2f}")
st.markdown("---")
...
with col1:
    ...
    st.selectbox("Bestemming", ["PRG", "PRS", "BLN", "DD", "BRU"], key='h_shift')
...
with col2:
    ...
    st.selectbox("Bestemming (Terug)", ["PRG", "PRS", "BLN", "DD", "BRU"], key='t_shift')
...
with tab4:
    if stat == "Extra (Horeca)":
        ...
        st.info(f"Flexi vakantiegeld (7.67% direct uitbetaald): € {totaal_vakantiegeld:.2f}")
    else:
        st.info("Geen direct uitbetaald vakantiegeld in dit statuut.")